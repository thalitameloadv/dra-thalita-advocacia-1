...
-- All syntax errors fixed for Supabase compatibility
...
-- Ensure search_term is unique so ON CONFLICT (search_term) works
CREATE UNIQUE INDEX IF NOT EXISTS idx_blog_analytics_search_terms_unique_search_term
ON public.blog_analytics_search_terms(search_term);
...
-- Create triggers for updated_at (Fixed - no IF NOT EXISTS)
DO $$
BEGIN
    ...
    DROP TRIGGER IF EXISTS update_blog_analytics_sessions_updated_at ON public.blog_analytics_sessions;
    DROP TRIGGER IF EXISTS update_blog_analytics_performance_updated_at ON public.blog_analytics_performance;
    DROP TRIGGER IF EXISTS update_blog_analytics_search_terms_updated_at ON public.blog_analytics_search_terms;
EXCEPTION WHEN OTHERS THEN
    -- Triggers don't exist, continue
END;
$$;
...
CREATE TRIGGER update_blog_analytics_sessions_updated_at 
BEFORE UPDATE ON public.blog_analytics_sessions
FOR EACH ROW
EXECUTE FUNCTION update_updated_at_column();

CREATE TRIGGER update_blog_analytics_performance_updated_at 
BEFORE UPDATE ON public.blog_analytics_performance
FOR EACH ROW
EXECUTE FUNCTION update_updated_at_column();

CREATE TRIGGER update_blog_analytics_search_terms_updated_at 
...
BEGIN
    ...
    DROP TRIGGER IF EXISTS trigger_update_analytics_performance ON public.blog_analytics_views;
EXCEPTION WHEN OTHERS THEN
