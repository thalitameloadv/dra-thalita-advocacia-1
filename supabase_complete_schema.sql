...
ADD COLUMN IF NOT EXISTS related_posts TEXT[] DEFAULT '{}';
...
-- Blog Categories
CREATE POLICY IF NOT EXISTS "Allow public read access on blog_categories" 
ON public.blog_categories FOR SELECT USING (true);

CREATE POLICY IF NOT EXISTS "Allow admin all access on blog_categories" 
ON public.blog_categories FOR ALL USING (auth.role() = 'authenticated');

-- Blog Posts
CREATE POLICY IF NOT EXISTS "Allow public read access on published blog_posts" 
ON public.blog_posts FOR SELECT USING (status = 'published');

CREATE POLICY IF NOT EXISTS "Allow admin read access on all blog_posts" 
ON public.blog_posts FOR SELECT USING (auth.role() = 'authenticated');

CREATE POLICY IF NOT EXISTS "Allow admin all access on blog_posts" 
ON public.blog_posts FOR ALL USING (auth.role() = 'authenticated');

-- Newsletter Subscribers
CREATE POLICY IF NOT EXISTS "Allow public insert on newsletter_subscribers" 
ON public.newsletter_subscribers FOR INSERT WITH CHECK (true);

CREATE POLICY IF NOT EXISTS "Allow admin all access on newsletter_subscribers" 
ON public.newsletter_subscribers FOR ALL USING (auth.role() = 'authenticated');

-- Newsletter Campaigns & Templates
CREATE POLICY IF NOT EXISTS "Allow admin all access on newsletter_campaigns" 
ON public.newsletter_campaigns FOR ALL USING (auth.role() = 'authenticated');

CREATE POLICY IF NOT EXISTS "Allow admin all access on newsletter_templates" 
ON public.newsletter_templates FOR ALL USING (auth.role() = 'authenticated');

CREATE POLICY IF NOT EXISTS "Allow admin read access on newsletter_analytics" 
...
CREATE POLICY IF NOT EXISTS "Allow authenticated read access on blog_analytics_sessions" 
ON public.blog_analytics_sessions FOR SELECT USING (auth.role() = 'authenticated');

CREATE POLICY IF NOT EXISTS "Allow service role write access on blog_analytics_sessions" 
ON public.blog_analytics_sessions FOR ALL USING (auth.role() = 'service_role');

CREATE POLICY IF NOT EXISTS "Allow authenticated read access on blog_analytics_views" 
ON public.blog_analytics_views FOR SELECT USING (auth.role() = 'authenticated');

CREATE POLICY IF NOT EXISTS "Allow service role write access on blog_analytics_views" 
ON public.blog_analytics_views FOR ALL USING (auth.role() = 'service_role');

CREATE POLICY IF NOT EXISTS "Allow authenticated read access on blog_analytics_engagement" 
ON public.blog_analytics_engagement FOR SELECT USING (auth.role() = 'authenticated');

CREATE POLICY IF NOT EXISTS "Allow service role write access on blog_analytics_engagement" 
ON public.blog_analytics_engagement FOR ALL USING (auth.role() = 'service_role');

CREATE POLICY IF NOT EXISTS "Allow authenticated read access on blog_analytics_traffic_sources" 
ON public.blog_analytics_traffic_sources FOR SELECT USING (auth.role() = 'authenticated');

CREATE POLICY IF NOT EXISTS "Allow service role write access on blog_analytics_traffic_sources" 
ON public.blog_analytics_traffic_sources FOR ALL USING (auth.role() = 'service_role');

CREATE POLICY IF NOT EXISTS "Allow authenticated read access on blog_analytics_performance" 
ON public.blog_analytics_performance FOR SELECT USING (auth.role() = 'authenticated');

CREATE POLICY IF NOT EXISTS "Allow service role write access on blog_analytics_performance" 
ON public.blog_analytics_performance FOR ALL USING (auth.role() = 'service_role');

CREATE POLICY IF NOT EXISTS "Allow authenticated read access on blog_analytics_search_terms" 
ON public.blog_analytics_search_terms FOR SELECT USING (auth.role() = 'authenticated');

CREATE POLICY IF NOT EXISTS "Allow service role write access on blog_analytics_search_terms" 
ON public.blog_analytics_search_terms FOR ALL USING (auth.role() = 'service_role');

CREATE POLICY IF NOT EXISTS "Allow authenticated read access on blog_analytics_events" 
ON public.blog_analytics_events FOR SELECT USING (auth.role() = 'authenticated');

CREATE POLICY IF NOT EXISTS "Allow service role write access on blog_analytics_events" 
...
-- You can run these queries to verify everything was created correctly:

-- Check tables
-- SELECT table_name FROM information_schema.tables WHERE table_schema = 'public' AND table_name LIKE '%blog%' OR table_name LIKE '%newsletter%' OR table_name LIKE '%analytics%';

-- Check categories
-- SELECT name, slug, icon, color FROM public.blog_categories ORDER BY name;

-- Check indexes
-- SELECT indexname FROM pg_indexes WHERE tablename IN ('blog_posts', 'blog_categories', 'newsletter_campaigns', 'blog_analytics_views');

-- Check RLS policies
-- SELECT tablename, policyname, permissive, roles FROM pg_policies WHERE tablename IN ('blog_posts', 'blog_categories', 'newsletter_campaigns');
